...
class BlockCompiler(code_provider.CodeProvider):
    ...
    def compile_block(self, codeline: int, baseindent: int, new_block = False) -> jsd:
        ...
        if line == "":
            ...
            return self.compile_block(codeline + 1, baseindent, new_block = new_block)
        elif new_block:
        ...
        if line.startswith("if "):
            ...
            if line.startswith("async "):
                exp = f"({name} := (lambda {args}: (({body.code}) for __ONE_trash in '_' if True or await fun()).__anext__())) and False"
            else:
                exp =   f"""
                            ({name} := lambda *args, **kvargs : (
                                [
